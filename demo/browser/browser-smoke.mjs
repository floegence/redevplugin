...
try {
  ...
  console.log("opaque browser demo smoke passed");
} finally {
  await browser.close();
  await demo.close();
}
...
async function verifyScenario(credentiallessScenario) {
  ...
  await page.goto(`${baseURL}/demo/browser/index.html?credentialless=${credentiallessScenario}`, { waitUntil: "domcontentloaded" });
  try {
    await page.waitForFunction(() => window.__redevpluginDemo?.snapshot().status === "ready", null, { timeout: 30_000 });
  } catch (error) {
    const failure = await page.evaluate(() => ({
      snapshot: window.__redevpluginDemo?.snapshot(),
  ...
  const snapshot = await page.evaluate(() => window.__redevpluginDemo.snapshot());
  ...
  const isolation = await frame.evaluate(async () => {
    ...
    return {
      ...
      parent_local_storage_blocked: await cannotReadHostSecret(() => Promise.resolve(localStorage.getItem("redevplugin_host_demo_secret"))),
      parent_session_storage_blocked: await cannotReadHostSecret(() => Promise.resolve(sessionStorage.getItem("redevplugin_host_demo_secret"))),
      ...
      direct_fetch_blocked: await blocked(() => fetch("/demo/browser/forbidden-direct-fetch")),
      service_worker_blocked: await blocked(async () => {
        const serviceWorker = navigator.serviceWorker;
        await serviceWorker.register("/demo/browser/forbidden-sw.js");
      }),
    };
  });
  ...
  await waitFor(async () => {
    const response = await fetch(`${baseURL}/demo/browser/diagnostics`);
    const value = await response.json();
    return value.asset_completed_at > 0;
  }, 5_000, "lazy asset completion");
  const diagnostics = await (await fetch(`${baseURL}/demo/browser/diagnostics`)).json();
  ...
  await page.waitForFunction(() => window.__redevpluginDemo.snapshot().status === "disposed");
  await waitFor(() => page.workers().length === 0, 5_000, "dedicated worker disposal");
  const disposed = await page.evaluate(() => window.__redevpluginDemo.snapshot());
