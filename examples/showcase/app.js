...
(() => {
  ...
  function isIncompleteSessionScopeResult(value) {
    if (!hasExactKeys(value, ["state", "fenced", "complete", "counts"]) || value.state !== "incomplete" || value.fenced !== true || value.complete !== false) return false;
  ...
  function registerPluginSurface(scope, pluginInstanceId, dispose, invalidate) {
    ...
    if (state.invalidated) {
      try {
        void Promise.resolve(invalidate()).catch(() => void 0);
      } catch {
      }
      return () => void 0;
    }
  ...
  async function disposePluginSurfaceScope(scope, pluginInstanceId) {
    ...
    const selected = [...state.registrations.entries()].filter(
  ...
  async function invalidatePluginSurfaceScope(scope) {
    const state = scopes.get(scope);
    if (!state) throw new TypeError("Plugin surface scope is invalid");
    ...
    const results = await Promise.allSettled(selected.map(
      (registration) => Promise.resolve().then(() => registration.invalidate())
    ));
    const failures = results.filter((result) => result.status === "rejected").map((result) => result.reason);
    if (failures.length === 1) throw failures[0];
  ...
  async function revokeSurfaceBootstrap(transport, bootstrap, timeoutMs, keepalive) {
    ...
    try {
      ...
      if (!isSurfaceRevocationReconciliation(result)) {
        throw new PluginBridgeError("PLUGIN_CONTRACT_MISMATCH", "Plugin surface revocation reconciliation returned an invalid result", void 0, void 0, "unknown");
      }
      return result;
    } catch (error) {
      if (timedOut) {
  ...
  var PluginSurfaceHostImplementation = class {
    ...
    dispose() {
      if (this.#disposed) return this.#revokeSurface(true).then(() => void 0);
      const revoke = this.#revokeSurface(true);
      void revoke.catch(() => void 0);
      this.#disposeLocal();
      return revoke.then(() => void 0);
    }
    ...
    async #handleExecutionQuery(message) {
      const controller = this.#registerPendingRequest(message.id);
      try {
        const snapshot = await this.#postJSON(
    ...
    #revokeSurface(keepalive) {
      ...
      void revocation.catch(() => {
        if (this.#revokePromise === revocation) this.#revokePromise = void 0;
      });
      return revocation;
    }
  ...
  function createPluginSurfaceOpeningLease(request2) {
    ...
    let invalidationObserver = () => void 0;
    const lease = {
    ...
    unregister = registerPluginSurface(
      request2.surfaceScope,
      request2.pluginInstanceId,
      () => lease.cancel(),
      () => lease.invalidate()
    );
  ...
  var PluginSurfaceSlot = class _PluginSurfaceSlot {
    ...
    #pendingRetirements = /* @__PURE__ */ new Set();
    #disposePromise;
    ...
    async #openSurface(options, controller, lease) {
      ...
      host.observeLocalInvalidation(() => {
        if (this.#active === host) this.#active = void 0;
        if (this.#opening === host) this.#opening = void 0;
    ...
    dispose() {
      ...
      const disposal = this.#tail.then(
        () => this.#closeCurrentSurface().then(() => void 0),
        () => this.#closeCurrentSurface().then(() => void 0)
      );
      const disposePromise = this.#unwrapCleanupError(disposal);
      this.#disposePromise = disposePromise;
      void disposePromise.catch(() => {
        if (this.#disposePromise === disposePromise) this.#disposePromise = void 0;
      });
    ...
    async #closeCurrentSurface() {
      const hosts = /* @__PURE__ */ new Set([
        ...
        ...[this.#active, this.#opening].filter((host) => host !== void 0)
      ]);
      this.#active = void 0;
      this.#opening = void 0;
      let result;
  ...
  function isSurfaceInteractionMessage(value) {
    ...
    return value.type === "redevplugin.surface.interaction" && validOpaqueHandle(value.frame_generation_id, "frame") && validOpaqueHandle(value.surface_handle, "surface") && typeof value.sequence === "number" && Number.isSafeInteger(value.sequence) && value.sequence > 0 && ["activation", "focus", "wheel", "text-selection", "action"].includes(String(value.kind)) && (value.target_key === null || validUIIdentifier(value.target_key)) && (value.action === null || validActionID(value.action)) && typeof value.local_scroll === "boolean" && typeof value.selection_active === "boolean" && (value.kind === "action" ? value.action !== null : value.action === null) && (value.kind === "wheel" || value.local_scroll === false) && (value.kind === "text-selection" || value.selection_active === false);
  }
  function isSurfaceRevocationReconciliation(value) {
    if (!hasExactKeys(value, ["disposed", "state", "previous_state", "revoked"])) return false;
    return value.disposed === true && ["closed", "absent"].includes(String(value.state)) && ["active", "closed", "absent"].includes(String(value.previous_state)) && typeof value.revoked === "boolean" && (value.revoked ? value.state === "closed" && value.previous_state === "active" : true);
  }
  ...
  function isExecutionCancelMessage(value) {
    return hasAllowedKeys(value, ["type", "id", "execution_id", "reason"]) && value.type === "redevplugin.bridge.execution.cancel" && validBridgeRequestID(value.id, "execution") && validOpaqueHandle(value.execution_id, "execution") && (value.reason === void 0 || typeof value.reason === "string" && value.reason.length <= 256);
  }
  function isExecutionQueryMessage(value) {
    return hasExactKeys(value, ["type", "id", "execution_id"]) && value.type === "redevplugin.bridge.execution.query" && validBridgeRequestID(value.id, "execution") && validOpaqueHandle(value.execution_id, "execution");
  }
  function isExecutionEventsMessage(value) {
    return hasExactKeys(value, ["type", "id", "execution_id", "after_cursor"]) && value.type === "redevplugin.bridge.execution.events" && validBridgeRequestID(value.id, "execution") && validOpaqueHandle(value.execution_id, "execution") && Number.isSafeInteger(value.after_cursor) && Number(value.after_cursor) >= 0;
  }
  ...
  var PluginPlatformClient = class {
    ...
    async installInspectedPackage(request2, options = {}) {
      ...
      try {
        ...
        await this.#handleMutationFailure(error, target || void 0, "External package install and local surface teardown failed");
        throw error;
      }
    ...
    listExecutionEvents(executionID, request2, options = {}) {
      return this.#requestQuery(`/_redevplugin/api/plugins/executions/${encodeURIComponent(executionID)}/events/query`, request2, options);
    }
    ...
    getPermissionRequirements(request2, options = {}) {
      return this.#requestQuery("/_redevplugin/api/plugins/permissions/requirements/query", request2, options);
    }
    ...
    async #handleMutationFailure(error, pluginInstanceId, message) {
      ...
      if (outcome === "not_committed" || outcome === void 0) return;
  ...
  function isPluginSessionScopeRevokeResult(value) {
    if (!isExactRecord(value, ["state", "fenced", "complete", "counts"]) || value.state !== "complete" || value.fenced !== true || value.complete !== true) return false;
    const counts = value.counts;
    if (!isExactRecord(counts, sessionScopeCountKeys2)) return false;
    return sessionScopeCountKeys2.every((key) => Number.isSafeInteger(counts[key]) && Number(counts[key]) >= 0);
  }
