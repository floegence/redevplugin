...
func TestExampleWorkerArtifactsUseCanonicalLinuxBuildAndSourceLock(t *testing.T) {
	...
	for _, required := range []string{
		...
		`--remap-path-prefix=/tmp/canonical-cargo-home=/cargo`,
		`"--network", "none"`,
		`hostWasmTarget},dst=${containerWasmTarget},readonly`,
		`hostCargoRegistry},dst=/tmp/canonical-cargo-home/registry,readonly`,
...
func TestScaffoldWorkerArtifactsUseCanonicalBuilderAndSourceLock(t *testing.T) {
	...
	if !strings.Contains(packageSource, `"build": "node scripts/build_surface_integrity.mjs --check && npm run contracts-package:build && tsc -p packages/redevplugin-ui/tsconfig.json && npm run browser-harness:generate && npm run scaffold:check"`) {
