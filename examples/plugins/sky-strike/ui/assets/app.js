...
(() => {
  ...
  function validAttribute(tag, name, value) {
    ...
    if (normalized === "maxlength" && (!/^(0|[1-9][0-9]*)$/.test(serialized) || !Number.isSafeInteger(Number(serialized)))) return false;
  ...
  var PluginBridgeClient = class {
    ...
    #keyboardInputHandlers = /* @__PURE__ */ new Set();
    ...
    async #handleMessage(event) {
      ...
      if (isPluginBridgeContextMessage(data)) {
        const context2 = normalizePluginSurfaceContext(data.context);
        if (this.#context && context2.revision <= this.#context.revision) return;
  ...
  function isPluginExecutionSnapshot(value) {
    ...
    const withProgress = (keys2) => hasAllowedKeys(value, [...keys2, "progress"]) && (value.progress === void 0 || validOperationProgress(value.progress));
