...
(() => {
    ...
    function validOperationProgress(value) {
        if (!isRecord(value) || !hasAllowedKeys(value, ["revision", "phase", "completed_units", "total_units", "unit"]) || !Number.isSafeInteger(value.revision) || Number(value.revision) < 1 || typeof value.phase !== "string" || !new RegExp("^[A-Za-z0-9._:-]{1,128}$").test(value.phase) || value.unit !== void 0 && (typeof value.unit !== "string" || !new RegExp("^[A-Za-z0-9._:-]{1,64}$").test(value.unit)) || value.completed_units !== void 0 && (!Number.isSafeInteger(value.completed_units) || Number(value.completed_units) < 0) || value.total_units !== void 0 && (!Number.isSafeInteger(value.total_units) || Number(value.total_units) < 0) || value.completed_units === void 0 !== (value.total_units === void 0))
            return false;
