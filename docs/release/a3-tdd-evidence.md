...
tag. `v0.3.1` validates the exact generated stress summary and produced the
signed GitHub Release, npm package, and four runtime bundles, but its final
published-release job still assumed checkout-root TypeScript dependencies and
stopped before registry readback. `v0.3.2` makes that verifier self-contained
and is the complete cross-artifact coordinate intended for host consumption.
...
| --- | --- | --- |
| Verified client generation | The new CLI tests returned the old usage string because `generate-client` still accepted an unsigned raw contract | `GOWORK=off go test ./cmd/redevplugin -run 'TestCLIHostCapabilityGenerateClientRequiresVerifiedBundleAndDetectsStaleOutput' -count=1` proves generation only reads a verified bundle and `--check` rejects stale output |
...
| External ingestion | Release capability requirements previously had metadata shape tests but no signed bundle resolver test | `GOWORK=off go test ./pkg/host -run 'TestResolveAndVerifyExternalHostCapabilityContract' -count=1` proves source-policy key usage/hash binding, no redirects, resolver/key separation, and exact pin verification |
| Host-owned handles | Business adapters previously selected operation and stream ids after dispatch | `GOWORK=off go test ./pkg/host ./pkg/operation ./pkg/stream -count=1` proves pre-dispatch Host allocation, scoped sinks, duplicate terminal rejection, route-local cancellation, revoke fencing, and durable cancel ownership |
...
| Review closure: generator and migrations | Numeric/boolean enum and const schemas widened to primitive types, non-cancelable operation clients exposed `cancel()`, and operation/stream SQLite tests did not start from v1 data | `GOWORK=off go test ./pkg/capabilitycontract ./pkg/operation ./pkg/stream -count=1` proves literal types, cancelability-specific operation types, cancelable subscriptions, v1 data/index migration, and idempotent reopen |
...
- Stream long polling is non-destructive until the final authorized mutation.
  ...
  ticket, and read failure preserves the old ticket and buffered events.
