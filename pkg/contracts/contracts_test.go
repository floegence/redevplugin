...
func TestContractInventoryIncludesSyntheticRegistry(t *testing.T) {
	...
	if len(artifacts) != 58 || len(registry.Artifacts) != len(artifacts) {
		t.Fatalf("artifact counts = %d/%d, want 58", len(artifacts), len(registry.Artifacts))
	}
