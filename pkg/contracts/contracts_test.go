...
func TestContractInventoryIncludesSyntheticRegistry(t *testing.T) {
	...
	if len(artifacts) != 59 || len(registry.Artifacts) != len(artifacts) {
		t.Fatalf("artifact counts = %d/%d, want 59", len(artifacts), len(registry.Artifacts))
	}
