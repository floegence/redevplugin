...
func TestContractInventoryIncludesSyntheticRegistry(t *testing.T) {
	...
	if len(artifacts) != 41 || len(registry.Artifacts) != len(artifacts) {
		t.Fatalf("artifact counts = %d/%d, want 41", len(artifacts), len(registry.Artifacts))
	}
