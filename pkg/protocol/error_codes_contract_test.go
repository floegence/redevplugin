...
func TestStableErrorCodeCatalogsMatchContracts(t *testing.T) {
	root := repoRoot(t)
	errorCodeSchema := readJSONMap(t, filepath.Join(root, "spec", "plugin", "error-codes-v6.schema.json"))
	...
	openAPIRaw, err := os.ReadFile(filepath.Join(root, "spec", "openapi", "plugin-platform-v8.yaml"))
	if err != nil {
		t.Fatal(err)
	}
