...
const generatedWorkerPath = resolve(root, "testdata/browser-harness/opaque-surface/generated/plugin-worker.js");
...
export function createBrowserHarnessServer(options = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      if (requestURL.pathname === "/__browser_harness/diagnostics" && request.method === "GET") {
      ...
      if (surfaceRoute) {
        ...
        if (surfaceRoute.action === "assets/read" && request.method === "POST") {
          if (body.asset_session !== surface.assetSession || body.asset_session_id !== surface.assetSessionID || body.binding_id !== "asset_harness_lazy_1") {
      ...
      if (requestURL.pathname === "/_redevplugin/api/plugins/confirm" && request.method === "POST") {
        ...
        writeEnvelope(response, {
          ...
          plan: {
            ...
            summary: "Run the harness protected action",
            risk_flags: [{ id: "harness-write", severity: "medium", summary: "Writes harness state", requires_confirmation: true }],
...
function serveStatic(rawPathname, response) {
  ...
  if (pathname === "/") pathname = "/testdata/browser-harness/opaque-surface/index.html";
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(`ReDevPlugin opaque browser harness: http://127.0.0.1:${port}/testdata/browser-harness/opaque-surface/index.html`);
  const shutdown = () => void harness.close().finally(() => process.exit(0));
