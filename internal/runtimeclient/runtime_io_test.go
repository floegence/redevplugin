...
func TestRuntimeIOControlUsesRawHostcallAndExactInvocation(t *testing.T) {
	request := []byte(`{"plugin_api":1,"operation":"storage.sqlite","arguments":{"operation":"exec","args":[{"float":52.52}]}}`)
	broker := &recordingRuntimeIOBroker{control: func(_ context.Context, invocationID string, raw []byte) ([]byte, error) {
		...
		return []byte(`{"ok":true,"result":{"latitude":52.52}}`), nil
	}}
	supervisor, generation, reader, cleanup := runtimeIOTestHarness(t, broker, map[string]context.Context{"invoke_a": context.Background()}, 1)
	defer cleanup()
	metadata, _ := json.Marshal(runtimeIOControlRequestMetadata{InvocationID: "invoke_a"})
	if err := supervisor.dispatchRuntimeIOFrame(generation, IPCFrame{Type: IPCFrameHostcall, RequestID: 90, Metadata: metadata, Body: request}); err != nil {
		t.Fatal(err)
	}
	response, err := ReadIPCFrame(reader)
	if err != nil {
		t.Fatal(err)
	}
	if response.Type != IPCFrameHostcallResult || response.RequestID != 90 || string(response.Body) != `{"ok":true,"result":{"latitude":52.52}}` {
...
func TestRuntimeIOReadUsesRawBodyAndExactInvocation(t *testing.T) {
	...
	supervisor, generation, reader, cleanup := runtimeIOTestHarness(t, broker, map[string]context.Context{"invoke_a": context.Background()}, 1)
	defer cleanup()
	metadata, _ := json.Marshal(runtimeIORequestMetadata{InvocationID: "invoke_a", ResourceID: 41, MaxBytes: 4})
	if err := supervisor.dispatchRuntimeIOFrame(generation, IPCFrame{
		Type: IPCFrameIORead, RequestID: 91, Metadata: metadata,
	}); err != nil {
		t.Fatal(err)
	}
	response, err := ReadIPCFrame(reader)
	...
	if response.Type != IPCFrameIOReadResult || response.RequestID != 91 || string(response.Body) != "data" {
	...
	if !result.OK || result.ResourceID != 41 || result.BytesRead != 4 || result.Flags != 1 {
		t.Fatalf("I/O result = %#v", result)
	}
}

func TestRuntimeIOReadPreservesZeroByteEOFOnWire(t *testing.T) {
	...
	metadata, _ := json.Marshal(runtimeIORequestMetadata{InvocationID: "invoke_a", ResourceID: 41, MaxBytes: 4})
	if err := supervisor.dispatchRuntimeIOFrame(generation, IPCFrame{
		Type: IPCFrameIORead, RequestID: 92, Metadata: metadata,
	}); err != nil {
		t.Fatal(err)
	}
	response, err := ReadIPCFrame(reader)
...
func TestRuntimeIOBlockingReadDoesNotStarveAnotherInvocation(t *testing.T) {
	...
	defer cancelA()
	supervisor, generation, reader, cleanup := runtimeIOTestHarness(t, broker, map[string]context.Context{
		"invoke_a": ctxA,
		"invoke_b": context.Background(),
	}, 2)
	defer cleanup()
	metadataA, _ := json.Marshal(runtimeIORequestMetadata{InvocationID: "invoke_a", ResourceID: 11, MaxBytes: 1})
	metadataB, _ := json.Marshal(runtimeIORequestMetadata{InvocationID: "invoke_b", ResourceID: 12, MaxBytes: 1})
	if err := supervisor.dispatchRuntimeIOFrame(generation, IPCFrame{Type: IPCFrameIORead, RequestID: 1, Metadata: metadataA}); err != nil {
		t.Fatal(err)
	}
	select {
	case <-blocked:
	...
	if err := supervisor.dispatchRuntimeIOFrame(generation, IPCFrame{Type: IPCFrameIORead, RequestID: 2, Metadata: metadataB}); err != nil {
		t.Fatal(err)
	}
	responseB, err := ReadIPCFrame(reader)
	if err != nil {
		t.Fatal(err)
	}
	...
	cancelA()
	responseA, err := ReadIPCFrame(reader)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRuntimeIORejectsUnknownInvocationBeforeBroker(t *testing.T) {
	broker := &recordingRuntimeIOBroker{}
	supervisor, generation, _, cleanup := runtimeIOTestHarness(t, broker, nil, 1)
	defer cleanup()
	metadata, _ := json.Marshal(runtimeIORequestMetadata{InvocationID: "invoke_unknown", ResourceID: 1, MaxBytes: 1})
	err := supervisor.dispatchRuntimeIOFrame(generation, IPCFrame{Type: IPCFrameIORead, RequestID: 1, Metadata: metadata})
...
func runtimeIOTestHarness(t *testing.T, broker RuntimeIOBroker, invocations map[string]context.Context, capacity int) (*ProcessSupervisor, *runtimeGeneration, io.Reader, func()) {
	t.Helper()
	reader, writer := io.Pipe()
	framed := newSemanticIPCWriteCloser(writer)
