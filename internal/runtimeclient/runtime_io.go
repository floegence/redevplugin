...
func isRuntimeIOControlRequestFrame(frame IPCFrame) bool {
	if frame.Type != IPCFrameHostcall || frame.Flags != 0 || len(frame.Body) == 0 || len(frame.Body) > int(IPCBodyMax) {
		return false
	}
	var probe struct {
...
func (s *ProcessSupervisor) dispatchRuntimeIOControlFrame(generation *runtimeGeneration, frame IPCFrame) error {
	...
	metadata.InvocationID = strings.TrimSpace(metadata.InvocationID)
	if metadata.InvocationID == "" || len(frame.Body) == 0 || len(frame.Body) > int(IPCBodyMax) {
	...
	go func() {
		...
		raw, err := s.ioBroker.Control(parent.ctx, metadata.InvocationID, frame.Body)
		if err != nil {
		...
		} else if len(raw) == 0 || len(raw) > int(IPCBodyMax) || !json.Valid(raw) {
...
func (s *ProcessSupervisor) executeRuntimeIO(ctx context.Context, request IPCFrame, metadata runtimeIORequestMetadata) IPCFrame {
	...
	result := runtimeIOResultMetadata{InvocationID: metadata.InvocationID, ResourceID: metadata.ResourceID, OK: true}
	var body []byte
	var err error
	switch request.Type {
	case IPCFrameIORead:
		body = make([]byte, metadata.MaxBytes)
		result.BytesRead, result.Flags, err = s.ioBroker.Read(ctx, metadata.InvocationID, metadata.ResourceID, body)
	...
	case IPCFrameIOWrite:
		result.BytesWritten, err = s.ioBroker.Write(ctx, metadata.InvocationID, metadata.ResourceID, request.Body, metadata.Flags)
	...
	case IPCFrameIOSeek:
		result.Offset, err = s.ioBroker.Seek(ctx, metadata.InvocationID, metadata.ResourceID, metadata.Offset, metadata.Whence)
	case IPCFrameIOClose:
		err = s.ioBroker.Close(ctx, metadata.InvocationID, metadata.ResourceID)
	}
	if err != nil {
		code, retryable := resourceio.StableError(err)
		result = runtimeIOResultMetadata{
			InvocationID: metadata.InvocationID, ResourceID: metadata.ResourceID, OK: false, Code: code, Message: strings.ToLower(strings.ReplaceAll(code, "_", " ")), Retryable: retryable,
		}
		body = nil
	}
	response.Metadata, _ = json.Marshal(result)
