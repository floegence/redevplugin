...
import {
  ...
  canonicalRustImage,
  canonicalWasmTargetSHA256,
  hashCanonicalWasmTarget,
...
test("canonical Docker builds use an immutable Rust image", () => {
  ...
  assert.equal(
    canonicalWasmTargetSHA256("1.88.0"),
    "3152d1cbc104215914e27562980c0d99b4164c7f1aa578794e003b16ce065c32",
  );
  assert.throws(() => canonicalWasmTargetSHA256("1.89.0"), /no pinned canonical WASM target/);
});

test("canonical Docker builds are offline and use read-only prepared inputs", async () => {
  const source = await readFile(new URL("./canonical_wasm_builder.mjs", import.meta.url), "utf8");
  assert.match(source, /"--network", "none"/);
  assert.match(source, /hostWasmTarget.*readonly/);
  assert.match(source, /hostCargoRegistry.*readonly/);
  assert.match(source, /cargo build --offline --locked/);
  assert.doesNotMatch(source, /rustup target add/);
});

test("canonical WASM target hashing is deterministic and rejects symlinks", async () => {
  const root = await mkdtemp(join(tmpdir(), "redevplugin-wasm-target-"));
  try {
    await mkdir(resolve(root, "lib/nested"), { recursive: true });
    await writeFile(resolve(root, "lib/a.rlib"), "alpha");
    await writeFile(resolve(root, "lib/nested/b.rlib"), "beta");
    const first = await hashCanonicalWasmTarget(root);
    const second = await hashCanonicalWasmTarget(root);
    assert.equal(first, second);
    await writeFile(resolve(root, "lib/a.rlib"), "tampered");
    assert.notEqual(await hashCanonicalWasmTarget(root), first);

    const symlinkResult = spawnSync("ln", ["-s", "a.rlib", resolve(root, "lib/alias.rlib")], { encoding: "utf8" });
    if (symlinkResult.status === 0) {
      await assert.rejects(hashCanonicalWasmTarget(root), /must not contain symlinks/);
    }
  } finally {
    await rm(root, { recursive: true, force: true });
  }
