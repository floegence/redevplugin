...
(() => {
  ...
  async function invokeStream() {
    await runAction("Opening runtime stream...", "Runtime stream received", async () => {
      const response = await bridge.call("worker.httpStream");
  ...
  async function invokeNetworkMatrix() {
  ...
  async function invokeDangerous() {
  ...
  async function runAction(starting, complete, action, failed = "Backend call failed") {
  ...
  function parseScheduleRows(value) {
    const record = value;
    const sqlite = record?.data?.storage_sqlite ?? record?.storage_sqlite;
  ...
  function decodePluginStreamText(event) {
  ...
  function render() {
    const scheduleMeta = state.scheduleRows.length > 0 ? `Persisted ${state.scheduleRows.length} item in ${state.scheduleRows[0].database}` : "No persisted schedule yet";
